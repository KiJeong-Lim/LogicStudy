...
\begin{document}
  ...
  \begin{context}[Solutions]
    \begin{enumerate}
      ...
      그러면, $F_1$과 $F_2$에는 개체 변수 $v_1$와 $v_2$만이 자유롭게 나타나며, $$F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \in \mathcal{T} \iff F_A \left[ v_1 := \overline{g \left( F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right)} \right] \in \mathcal{T}$$이고
      $$F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \in \mathcal{T} \iff F_B \left[ v_1 := \overline{g \left( F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right)} \right] \in \mathcal{T}$$이다.
      ...
      \begin{align*}
        X \in \mathcal{T}
        & \iff F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \in \mathcal{T} \\
        & \iff F_A \left[ v_1 := \overline{g \left( F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right)} \right] \in \mathcal{T} \\
        & \iff g \left( F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right) \in A \\
        & \iff g \left( Y \right) \in A
      \end{align*}
      이고,
      \begin{align*}
        Y \in \mathcal{T}
        & \iff F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \in \mathcal{T} \\
        & \iff F_B \left[ v_1 := \overline{g \left( F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right)} \right] \in \mathcal{T} \\
        & \iff g \left( F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right) \in B \\
        & \iff g \left( X \right) \in B
      \end{align*}
      이다.
      이로써 $$X \in \mathcal{T} \leftrightarrow g \left( Y \right) \in A$$와 $$Y \in \mathcal{T} \leftrightarrow g \left( X \right) \in B$$가 성립함을 알 수 있다.
    \end{enumerate}
  \end{context}
  ...
  \textbf{Group I--Axiom Schemes for Propositional Logic}
  \begin{enumerate}
  ...
  \textbf{Group II--Additional Axiom Schemes for First-Order Logic with Identity]}
  \begin{enumerate}
    ...
    {
      대안적으로, 이 공리꼴은 $\left( v_i = t \rightarrow \left( Y_1 \rightarrow Y_2 \right) \right)$로 작성될 수 있다.
      여기서 $Y_1$은 원자 논리식이고, $Y_2$는 $Y_1$에서 $v_i$의 나타남 중 아무거나 하나를 $t$로 교체하여 얻은 식이다.
    }
  \end{enumerate}

  \textbf{Group III--Eleven Axiom Schemes Having Only One Axiom Apiece}
  \begin{enumerate}
  ...
  이 공리꼴을 진열하기 위하여, $F$를 $v_1$만이 자유롭게 나타나는 논리식이라고 하자.
  $F \left( v_1 ' \right)$를 $$\forall v_i \left( v_i = v_1 ' \rightarrow \forall v_1 \left( v_1 = v_i \rightarrow F \right) \right)$$ 꼴의 아무 논리식이라고 하되,
  ...
  이때,
  \begin{enumerate}
    \item[$N_{12}$ :] $\left( F \left( \overline{0} \right) \rightarrow \left( \left( \forall v_1 F \rightarrow F \left( v_1 ' \right) \right) \rightarrow \forall v_1 F \right) \right)$
  \end{enumerate}
  가 수학적 귀납법의 공리꼴이다.
