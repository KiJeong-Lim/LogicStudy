...
$H \left( v_1 \right)$의 대각화 $H \left[ \bar{h} \right]$은 집합 $\widetilde{P_E}$에 대한 괴델 문장이다.
이 문장은 P.E. 안에서 증명 가능하지 않을 때 그리고 그럴 때에만 참이다.
P.E.가 정확한 체계이기 때문에 $H \left[ \bar{h} \right]$은 반드시 참이지만 P.E.에서 증명 가능하지 않다.
또한 $ \lnot H \left[ \bar{h} \right] $은 거짓이므로, 이 문장도 P.E.에서 증명 가능하지 않다.
\end{document}
