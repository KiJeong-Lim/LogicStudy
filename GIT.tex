...
\begin{context}[Exercise 1]
$\mathrm{Fr} \left( x , y \right)$를 ``$E_x$이 변수이고, $E_y$가 논리식이며, $E_x$는 $E_y$에서 최소한 한 번 자유롭게 나타난다''는 관계라고 하자.\footnote
{
...
연습문제 1을 이용하여, 보여라:
\begin{enumerate}
\item \textit{문장}의 괴델 수들의 집합이 산술적이다;
\item P.E.의 증명 가능한 \textit{문장}들의 괴델 수들의 집합이 산술적이다.\footnote
{
...
\begin{enumerate}
\item $E$가 숫자 또는 $w$와 다른 변수일 때 $E \left[ w \backslash t \right] = E$이지만,
$E = w$이면 $E \left[ w \backslash t \right] = t$이다. 즉, $w \left[ w \backslash t \right] = t$.
\item $E$가 $r + s$, $r \times s$, $r \, \mathbf{E} \, s$ 또는 $r^{\prime}$이면
$E \left[ w \backslash t \right]$는 각각 $r \left[ w \backslash t \right] + s \left[ w \backslash t \right]$,
$r \left[ w \backslash t \right] \times s \left[ w \backslash t \right]$, $ r \left[ w \backslash t \right] \, \mathbf{E} \, s \left[ w \backslash t \right]$ 또는 $r \left[ w \backslash t \right]^{\prime}$이다.
\item $E$가 원자논리식 $r = s$ 또는 $r \le s$이면
$E \left[ w \backslash t \right]$는 각각 $ r \left[ w \backslash t \right] = s \left[ w \backslash t \right] $ 또는 $ r \left[ w \backslash t \right] \le s \left[ w \backslash t \right] $이다.
\item $E$가 논리식 $F \rightarrow G$ 또는 $\lnot F$이면,
$E \left[ w \backslash t \right]$는 각각
$F \left[ w \backslash t \right] \rightarrow G \left[ w \backslash t \right]$ 또는
$\lnot \left[ w \backslash t \right]$이다.
\item $E$가 논리식 $\forall v F$이면( 여기서 $v$는 $w$와 다른 변수이다 ),
$E \left[ w \backslash t \right] = \forall v F \left[ w \backslash t \right]$이다.
그러나 $E$가 $\forall w F$이면 $E \left[ w \backslash t \right] = E$이다. 
\end{enumerate}
이제 $\mathrm{Sub} \left( E , w , t , F \right)$를 ``$E$가 논리항이거나 논리식이고 $w$는 변수이고 $t$는 논리항인데 $F = E\left[ w \backslash t \right]$이다''라는 관계로 두자.
...
\item [$L_5^{\prime}$]: $\forall w \left( F \rightarrow F \left[ w \backslash t \right] \right)$,
...
(g) & $ \left( v_1 = v_2 \rightarrow \left( v_3 = v_4 \rightarrow \left( v_1 \le v_3 \rightarrow v_2 = v_4 \right) \right) \right) $ \\ \hline
\end{tabular}
\end{enumerate}
