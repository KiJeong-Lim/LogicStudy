...
\begin{document}
  ...
  모든 $\mathcal{N}$의 부분 집합들의 집합은 비가산 집합이다.
  ...
  \begin{context}[G\"odel Numbering and Diagonalization]
    $g$를 각 $E \in \mathcal{E}$에게 \underline{괴델 수}(\textit{G\"odel number})라고 불리는 자연수 $g \left( E \right)$를 부여하는 단사 함수라고 하자.
    이 단원의 나머지 부분에서 $g$는 고정될 것이다.\footnote
    {
      뒷 단원들에서 공부할 구체적인 체계에서는 특정 괴델 넘버링이 주어질 것이다.
      지금 다루는 순수하게 추상적인 논의에서는 임의의 괴델 넘버링에 대하여 적용될 수 있다.
    }
    $g$가 전사함수라고 가정하는 것이 기술적으로 편리할 것이다.\footnote
    {
    ...
    즉, 임의의 $n \in \mathbb{N}$에 대하여 $g \left( E_{n} \right) = n$이다.
    ...
    즉, $A^{*}$의 정의에 의하여 동등성 $$ \left( \forall n \in \mathbb{N} \right) \left[ n \in A^{*} \leftrightarrow g \left( E_{n} \left( n \right) \right) \in A \right] $$이 성립한다.\footnote
    {
  ...
  \begin{context}[G\"odel Sentences]
    ...
    $$X \in \mathcal{T} \leftrightarrow g \left( X \right) \in A$$가 성립하면,
  ...
  \begin{proof}
    \begin{itemize}
      ...
      그런데 $g \left( H \left( h \right) \right) = d \left( h \right)$이므로,
  ...
  \begin{context}[Exercise 5]
    괴델 넘버링 $g$가 전사 함수가 아닌 경우를 고려하자.
    이때 $$ \left( \forall E \in \mathcal{E} \right) \left( \forall e \in \mathbb{N} \right) \left[ g \left( E \right) = e \rightarrow g \left( \Phi \left( E , e \right) \right) = d \left( e \right) \right] $$를
  ...
  \begin{context}[Solutions]
    \begin{enumerate}
      ...
      이제, $h^{\prime} := g \left( H^{\prime} \right)$에 대하여, $H \left( h^{\prime} \right)$이 $\mathcal{L}$에서 증명도 반증도 가능하지 않다는 것을 보일 것이다.
      ...
      이제, $k := g \left( K \right)$에 대하여, $K \left( k \right)$가 $\mathcal{L}$에서 증명도 반증도 가능하지 않다는 것을 보일 것이다.
      ...
      이제, $k := g \left( K \right)$에 대하여, 문장 $K \left( k \right)$가 $\mathcal{L}$에서 결정 불가능함을 보이겠다.
      ...
      이제, $h := g \left( H \right)$에 대하여, $H \left( h \right)$가 $A$에 대한 괴델 문장이라는 것을 보일 것이다.
      먼저, $h = g \left( H \right)$이므로,
      ...
      \begin{align*}
        ...
        & \iff g \left( H \left( h \right) \right) \in A
      \end{align*}
      의 동등성을 얻는데,
      ...
      \begin{itemize}
        ...
        \begin{align*}
          ...
          & \iff 66 \notin P^{*} \\
          & \iff d \left( 66 \right) \notin P^{*} \\
          & \iff g \left( E_{66} \left( 66 \right) \right) \notin P \\
        ...
        \begin{align*}
          ...
          & \iff g \left( E_{a_{n}} \left( b_{n} \right) \right) \in \tilde{P} \\
          & \iff g \left( E_{a_{n}} \left( b_{n} \right) \right) \notin P \\
        ...
        \begin{align*}
          ...
          & \iff g \left( E_{31} \left( 31 \right) \right) \in A
        \end{align*}
        인데,
  ...
  그러나 $f \circ$을 ``$+$''로, $f \! \circ \! \circ$을 ``$\times$''로, $f \! \circ \! \circ \circ$을 ``$\mathbf{E}$''로 줄여서 표기하자.
  ...
  \begin{context}[Abbreviations]
    ...
    \begin{align*}
      ...
      \left( t_{1}^{t_{2}} \right) & : \equiv \left( t_{1} \mathop{\mathbf{E}} t_{2} \right) \\
  ...
  \noindent \textbf{\S 2. $\mathcal{L}_{E}$에서의 참의 개념.}
  ...
  더 일반적으로는, $m *_{10} n = m^{\mathrm{len}_{10} \left( n \right)} + n$이다 --
  ...
  이때 함수 $n \mapsto g \left( \overline{n} \right) : \mathbb{N} \to \mathbb{N}$이 싼술적이기를 원한다.
  당연히, $\overline{n}$은 ``$0$'' 뒤에 $n$개의 악센트 기호가 붙은 것이므로,
  그것의 괴델 수를 13진법으로 표기하면 ``$1$'' 뒤에 ``$0$''이 $n$번 나타나는 문자열과 같다.
  따라서 $g \left( \overline{n} \right) = 13^{n}$이다.
  ...
  \begin{context}[Discussion]
    ...
    \begin{enumerate}
      ...
      \item 함수 $n \mapsto g \left( \overline{n} \right) : \mathbb{N} \to \mathbb{N}$가 싼술적이다.
    \end{enumerate}
  ...
  당연히, 기호들에게 다음과 같은 괴델 수를 부여함으로써, 13 대신 10을 괴델 넘버링의 기저로 둘 \textit{수 있었고}, $$ \begin{matrix}
    0 \\
    1
  \end{matrix} \quad
  \begin{matrix}
  ...
  집합 $T \subseteq \mathbb{N}$을 $T := \left\{ g \left( X \right) : X \in \mathcal{T} \right\}$으로 놓자.
  ...
  이제 임의의 싼술적인 집합 $A \subseteq \mathbb{N}$에 대하여
  (Tarski의 정리가 쉽게 따라오도록) $A$에 대한 괴델 문장이 있음을 보이는 데 초점을 맞추겠다.

  거의 모든 책이 괴델 문장을 구성하는 여러 가지 영리한 방법으로 쓰여질 수 있다.
  G\"odel의 원래 방법은 임의의 논리식 $F$와 $G$ 그리고 임의의 수 $n$에 대하여,
  $$F \left[ v_1 := \overline{n} \right] \equiv G \rightarrow \mathrm{sub} \left( g \left( F \right) , n \right) = g \left( G \right)$$
  ...
  함수 $r : \mathbb{N} \times \mathbb{N} \to \mathbb{N}$을 $$ r \left( x , y \right) := g \left( E_x \left( y \right) \right) $$으로 정의하자.
  ...
  \begin{context}[Diagonalization]
    ...
    임의의 $n \in \mathbb{N}$에 대하여 $d \left( n \right) = g \left( E_{n} \left( n \right) \right)$이다.
  \end{context}
  ...
  집합 $T$와는 다르게 체계에서 \textit{증명 가능}한 모든 문장들의 괴델 수를 모은 집합은 싼술적이라는 것이 밝혀질 것이다.
  ...
  \begin{context}[Solutions]
    \begin{enumerate}
      ...
      \begin{itemize}
        ...
        \item[(b)] 논리식 $\forall v_2 \forall v_3 \left( \left( v_1 = v_2 \times v_3 \right) \rightarrow \left( v_2 = \overline{1} \lor v_2 = v_1 \right) \right)$이 모든 소수들의 집합을 표현하기 때문이다.
      \end{itemize}
      ...
      \begin{itemize}
        ...
        \begin{align*}
          ...
          F_2 & : \equiv \forall v_4 \left( F_f \left[ v_2 := v_4 \right] \rightarrow F_g \left[ v_2 := v_4 \right] \right) \\
          F_3 & : \equiv \forall v_4 \left( F_g \left[ v_3 := v_4 \right] \rightarrow F_f \left[ v_1 := v_4 \right] \right)
        \end{align*}
      ...
      $g \left( \overline{x} \right) = 10^{x}$임과 $$ g \left( v_1 \right) = g \left( \left( v \circ \right) \right) = 2 *_{10} 6 *_{10} 5 *_{10} 3 = 2653 $$임에 주목하면,
      \begin{align*}
        d \left( x \right)
        & = r \left( x , x \right) \\
        & = g \left( \forall v_1 \left( v_1 = \overline{x} \rightarrow E_x \right) \right) \\
        & = g \left( \forall \right) *_{10} g \left( v_1 \right) *_{10} g \left( ( \right) *_{10} g \left( v_1 \right) *_{10} g \left( = \right) *_{10} g \left( \overline{x} \right) *_{10} g \left( \rightarrow \right) *_{10} g \left( E_x \right) * g \left( ) \right) \\
        & = 899 *_{10} 2653 *_{10} 2 *_{10} 2653 *_{10} 10^{x} *_{10} 89 *_{10} x *_{10} 3 \\
        & = 899265322653 *_{10} 10^{x} *_{10} 89 *_{10} x *_{10} 3
      \end{align*}
      를 얻는다. $k := 899265322653$로 놓자.
      \begin{itemize}
        \item[(a)] 이제 모든 짝수들의 집합을 $A$라고 하자.
        임의의 $x \in \mathbb{N}$에 대하여
        \begin{align*}
          x \in A^{*}
        ...
        \end{align*}
        이므로,
        논리식 $$ \exists v_2 \left( \overline{k} *_{10} \overline{10}^{v_1} *_{10} \overline{89} *_{10} v_1 *_{10} \overline{3} = v_2 \land \exists v_3 \left( v_2 = \overline{2} \times v_3 \right) \right) $$은 집합 $A^{*}$를 표현한다.
        이 논리식의 괴델 수를 $h$라 하자.
        그러면 $$ \forall v_1 \left( v_1 = \overline{h} \rightarrow \exists v_2 \left( \overline{k} *_{10} \overline{10}^{v_1} *_{10} \overline{89} *_{10} v_1 *_{10} \overline{3} = v_2 \land \exists v_3 \left( v_2 = \overline{2} \times v_3 \right) \right) \right) $$은 $A$의 괴델 문장이다.
        $\overline{k} *_{10} \overline{10}^{\overline{h}} *_{10} \overline{89} *_{10} \overline{h} *_{10} \overline{3}$가 짝수일 때 그리고 그럴 때에만 이 문장이 참인데,
        이 수의 $10$진 표기의 마지막 숫자는 $3$이므로 그것은 참이지 않음을 알 수 있다.
        \item[(b)] 이제 모든 홀수들의 집합을 $A$라고 하자.
        임의의 $x \in \mathbb{N}$에 대하여
        \begin{align*}
          x \in A^{*}
          & \iff \left( \exists y \in \mathbb{N} \right) \left[ d \left( x \right) = y \land y \in A \right] \\
        ...
        \end{align*}
        이므로,
        논리식 $$ \exists v_2 \left( \overline{k} *_{10} \overline{10}^{v_1} *_{10} \overline{89} *_{10} v_1 *_{10} \overline{3} = v_2 \land \forall v_3 \lnot \left( v_2 = \overline{2} \times v_3 \right) \right) $$은 집합 $A^{*}$를 표현한다.
        이 논리식의 괴델 수를 $h$라 하자.
        그러면 $$ \forall v_1 \left( v_1 = \overline{h} \rightarrow \exists v_2 \left( \overline{k} *_{10} \overline{10}^{v_1} *_{10} \overline{89} *_{10} v_1 *_{10} \overline{3} = v_2 \land \forall v_3 \lnot \left( v_2 = \overline{2} \times v_3 \right) \right) \right) $$은 $A$의 괴델 문장이다.
        $\overline{k} *_{10} \overline{10}^{\overline{h}} *_{10} \overline{89} *_{10} \overline{h} *_{10} \overline{3}$가 홀수일 때 그리고 그럴 때에만 이 문장이 참인데,
        이 수의 $10$진 표기의 마지막 숫자는 $3$이므로 그것은 참임을 알 수 있다.
      \end{itemize}
      ...
      $F \left( g \left( F \right) \right)$은 $E_{n}$이 표현하는 집합의 괴델 문장이다.
      그러므로 $$ f \left( n \right) := g \left( \forall v_1 \left( v_1 = \overline{g \left( F \right)} \rightarrow F \right) \right) $$로 두자.
      ...
      표현식 $E$에 대하여 각 $x \in \mathbb{N}$와 각 $y \in \mathbb{N}$마다 $$E \left( x , y \right) : \equiv \forall v_1 \forall v_2 \left( v_1 = \overline{x} \rightarrow \left( v_2 = \overline{y} \rightarrow E \right) \right)$$라고 표기하자.
      그리고 함수 $\mathrm{fst} : \mathbb{N} \times \mathbb{N} \to \mathbb{N}$와 $\mathrm{snd} : \mathbb{N} \times \mathbb{N} \to \mathbb{N}$을
      각각 $$ \mathrm{fst} \left( x , y \right) := g \left( E_x \left( x , y \right) \right)$$와
      $$\mathrm{snd} \left( x , y \right) := g \left( E_y \left( x , y \right) \right)$$로 두자.
      ...
      그러면 $$F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \in \mathcal{T} \leftrightarrow F_A \left[ v_1 := \overline{g \left( F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right)} \right] \in \mathcal{T}$$이고
      $$F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \in \mathcal{T} \leftrightarrow F_B \left[ v_1 := \overline{g \left( F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right) \right)} \right] \in \mathcal{T}$$이다.
      이때 $X : \equiv F_1 \left( g \left( F_1 \right) , g \left( F_2 \right) \right)$와 $Y : \equiv F_2 \left( g \left( F_1 \right) , g \left( F_2 \right) \right)$를 취하자.
      그러면 $$X \in \mathcal{T} \iff g \left( Y \right) \in A$$이고 $$Y \in \mathcal{T} \iff g \left( X \right) \in B$$이다.
    \end{enumerate}
  \end{context}
